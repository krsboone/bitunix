"""
bb_limit_dir_trader.py — BB Momentum trader: limit entry + limit TP + directional bias

Same as bb_limit_trader.py with one addition:
  - --dir argument gates which signal directions are allowed to trade.
  - LONG  → only LONG signals fire; SHORT signals are filtered.
  - SHORT → only SHORT signals fire; LONG signals are filtered.
  - BOTH  → no filter; both directions trade normally (default).

Rationale: in a trending market, restricting to the trend direction eliminates
counter-trend noise entries. Combine with the extended MAX_HOLD_MINS to give
trend-aligned trades room to develop.

Hold time is extended to 240 minutes (from 60) to reduce TIME exits on trades
that are directionally correct but slow-moving.

State machine:
  WATCHING → ENTRY_PENDING → LIMIT_OPEN → IN_TRADE → WATCHING

Usage:
    python3 bb_limit_dir_trader.py                       # live, both directions
    python3 bb_limit_dir_trader.py --debug               # DEBUG mode — no real orders
    python3 bb_limit_dir_trader.py --debug --dir LONG    # long-only in debug
    python3 bb_limit_dir_trader.py --debug --dir SHORT   # short-only in debug
    python3 bb_limit_dir_trader.py --symbol BTCUSDT
"""
...
start_logging("bb_limit_dir")

STRATEGY = "bb_limit_dir"
...
TRADE_CSV = os.path.join("log", "bb_limit_dir_trades.csv")
...
def _process_symbol(client: BitunixClient, sym: str, s: dict,
    ...
    if signal_side == "LONG":
        if dir_filter == "SHORT":
            log.info(f"  {sym}: LONG signal FILTERED — dir={dir_filter}")
            arm_log.log_arm_event(
                arm_log.new_arm_id(), STRATEGY, sym,
                now_utc().strftime("%Y-%m-%d %H:%M:%S"), price, "LONG",
                "NO_FIRE", no_fire_reason="DIR_FILTER", atr=hw,
            )
        else:
        ...
        if dir_filter == "LONG":
            log.info(f"  {sym}: SHORT signal FILTERED — dir={dir_filter}")
            arm_log.log_arm_event(
                arm_log.new_arm_id(), STRATEGY, sym,
...
def run(debug: bool, symbols: list[str] = None, dir_filter: str = "BOTH") -> None:
    ...
    log.info("━" * 60)
    log.info("  BB Momentum Trader — Limit Orders + Directional Filter")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="BB Momentum trader — limit entry + limit TP + directional filter")
