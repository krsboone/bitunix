"""
trend_trader.py — Bitunix trend-filtered perpetual futures trader
...
Strategy:
  ...
  2. Trend bias gate:
       - Current price BELOW trend line → LONG signals only
       - Current price ABOVE trend line → SHORT signals only
...
Usage:
    ...
    python3 trend_trader.py --debug      # DEBUG mode (no real orders)
"""
...
LOOKBACK_DAYS  = 60            # daily candles to analyse
...
TRADE_CSV = os.path.join("log", "trend_trades.csv")
...
def run(debug: bool) -> None:
    ...
    log.info("━" * 62)
    log.info("  Bitunix Trend + Brownian Motion Trader")
    ...
    if not debug:
        for p in get_open_positions(client):
            ...
            log.info(f"  Hydrated {sym} [{side}]  entry={tracked[sym]['entry_price']:.4f}")

    # Cache trend lines — recompute every TREND_REFRESH_CYCLES cycles
        ...
        try:
            ...
            for sym in SYMBOLS:
                ...
                try:
                    ...
                    bias      = "SHORT" if above else "LONG"

                    log.info(f"  {sym}  price={price:.4f}  "
                             f"trend={trend_val:.4f}  "
                             f"{'above' if above else 'below'} → bias={bias}  "
                    ...
                    # Trend gate — signal must match bias
                    if side != bias:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Bitunix trend-filtered Brownian motion trader")
