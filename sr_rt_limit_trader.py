"""
sr_rt_limit_trader.py — S/R Breakout trader with retest entry + limit TP

Same strategy as sr_retest_trader.py. Differences:
  - Entry:  limit order at retest confirmation price  → maker fee on open
...
State machine:
  WATCHING → ARMED → RETEST → LIMIT_OPEN → IN_TRADE → WATCHING

Usage:
    python3 sr_rt_limit_trader.py              # live trading
    python3 sr_rt_limit_trader.py --debug      # DEBUG mode — no real orders
    python3 sr_rt_limit_trader.py --symbol BTCUSDT
"""
...
start_logging("sr_rt_limit_trader")

STRATEGY = "sr_rt_limit"

# ── Per-symbol sweep-optimised config (same as sr_retest_trader.py) ───────────
...
TRADE_CSV = os.path.join("log", "sr_rt_limit_trades.csv")
...
def run(debug: bool, symbols: list[str] = None) -> None:
    ...
    log.info("━" * 60)
    log.info("  S/R Breakout Trader (Retest) — Limit Orders")
    log.info(f"  Symbols   : {', '.join(active)}")
    log.info(f"  TP/SL     : {TP_MULT}/{SL_MULT}σ  |  Hold: ≤{MAX_HOLD_MINS}min")
    log.info(f"  Z-entry   : {Z_ENTRY}  Z-retest: {Z_RETEST}  |  Leverage: {LEVERAGE}×")
...
def _enter_trade(client: BitunixClient, sym: str, s: dict, cfg: dict,
    ...
    """Place a limit entry order at the retest confirmation price."""
    if sigma == 0:
    ...
    # retest_start preserved (informational)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="S/R Breakout trader with retest — limit entry + limit TP")
