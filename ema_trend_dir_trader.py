"""
ema_trend_dir_trader.py — EMA Trend trader + directional filter

Same as ema_trend_trader.py with the following changes:

  EMA periods: 9/21 → 21/55
    Fewer whipsaw signals on 15m charts. The 9/21 pair re-crosses on every
    minor pullback in a trend; 21/55 stays crossed through normal noise.

  ADX threshold: 20 → 25
    20 is barely trending. 25 is the conventional floor for a confirmed trend.
    Raises signal quality at the cost of fewer entries.

  LOCK_ATR: 0.25 → 1.0
    Once breakeven activates, SL moves to entry + 1×ATR. The original 0.25×ATR
    locked in ~$75 on BTC — less than typical noise. 1.0×ATR locks in a
    meaningful gain while still allowing normal pullbacks to breathe.

  --dir argument: LONG, SHORT, or BOTH (default BOTH)
    Gates which crossover directions are allowed to trade. In a trending
    market, LONG eliminates counter-trend whipsaw entries from short crosses.

State machine:
  WATCHING → ENTRY_PENDING → IN_TRADE → WATCHING

Usage:
    python3 ema_trend_dir_trader.py                       # live, both directions
    python3 ema_trend_dir_trader.py --debug               # DEBUG mode — no real orders
    python3 ema_trend_dir_trader.py --debug --dir LONG    # long-only in debug
    python3 ema_trend_dir_trader.py --debug --dir SHORT   # short-only in debug
    python3 ema_trend_dir_trader.py --symbol BTCUSDT
"""
...
start_logging("ema_trend_dir")

STRATEGY = "ema_trend_dir"
...
ATR_PERIOD      = 14
ADX_PERIOD      = 14
ADX_THRESHOLD   = 25     # raised from 20 — require confirmed trend before entering
SL_MULT         = 2.0
TP_MULT         = 5.0
MAX_HOLD_MINS   = 360
BREAKEVEN_ATR   = 1.0    # profit must reach 1× ATR to activate breakeven stop
LOCK_ATR        = 1.0    # raised from 0.25 — lock in 1× ATR above entry
...
TRADE_CSV = os.path.join("log", "ema_trend_dir_trades.csv")
...
def _process_symbol(client: BitunixClient, sym: str, s: dict,
                    balance: float, debug: bool, dir_filter: str = "BOTH") -> None:
    ...
    # Directional filter
    if dir_filter != "BOTH" and signal_side != dir_filter:
        log.info(f"  {sym}: CROSS {signal_side} filtered — dir={dir_filter}")
        arm_log.log_arm_event(
            arm_log.new_arm_id(), STRATEGY, sym,
            now_utc().strftime("%Y-%m-%d %H:%M:%S"), close, signal_side,
            "NO_FIRE", no_fire_reason="DIR_FILTER", atr=atr,
        )
        return
...
def _monitor_trade(client: BitunixClient, sym: str, s: dict,
    ...
    """Monitor open position; exit on EMA re-cross, time limit, SL, or TP."""
    pos       = s["position"]
...
def run(debug: bool, symbols: list[str] = None, dir_filter: str = "BOTH") -> None:
    ...
    log.info("━" * 60)
    log.info("  EMA Trend Trader — Directional Filter")
    ...
    log.info(f"  ATR period  : {ATR_PERIOD}  |  ADX threshold: {ADX_THRESHOLD}")
    log.info(f"  SL/TP       : {SL_MULT}/{TP_MULT}× ATR  (primary exit: EMA re-cross)")
    log.info(f"  Breakeven   : trigger={BREAKEVEN_ATR}×ATR  lock={LOCK_ATR}×ATR above entry")
    ...
    while True:
        ...
        try:
            ...
            for sym in active:
                try:
                    _process_symbol(client, sym, sym_state[sym], balance, debug, dir_filter)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EMA Trend Trader — directional filter")
    ...
    parser.add_argument("--dir", choices=["LONG", "SHORT", "BOTH"], default="BOTH",
                        metavar="DIR",
                        help="Directional filter: LONG, SHORT, or BOTH (default: BOTH)")
    args = parser.parse_args()
    run(args.debug, [args.symbol] if isinstance(args.symbol, str) else args.symbol,
        dir_filter=args.dir)
