...
SL_MULT        = 1.0    # 2026-04-23: changed from 3.0 — was causing ~1:5 realized R:R
Z_ENTRY        = 1.2
...
def _enter_trade(client: BitunixClient, sym: str, s: dict, cfg: dict,
    ...
    position_registry.register(position_id, STRATEGY, sym, side, entry, saved_arm_id)
...
def _monitor_trade(client: BitunixClient, sym: str, s: dict,
    ...
    if not live:
        ...
        if hist:
            close_px = float(hist.get("closePrice", c["close"]))
        else:
            close_px = c["close"]
