...
def _enter_pending(client: BitunixClient, sym: str, s: dict,
    ...
    # orderId ≠ positionId on Bitunix — resolve the actual position ID so
    # _monitor_trade can match it against get_open_positions() correctly.
    if debug:
...
def _monitor_trade(client: BitunixClient, sym: str, s: dict,
    ...
    if not live:
        # Query actual close price from history — more reliable than candle inference
